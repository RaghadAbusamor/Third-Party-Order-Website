...
from users.models import Customer, Store, Delivery,User,StoreEvaluation
...
class StoreSignUpForm(CustomUserCreationForm):
    username = forms.CharField(label='Full User Name',max_length=50)
    password1 = forms.CharField(label='password', widget=forms.PasswordInput)  
    password2 = forms.CharField(label='Confirm password', widget=forms.PasswordInput)  
    phone_number = forms.CharField(max_length=16, validators=[
...
class DeliverySignUpForm(CustomUserCreationForm):
    username = forms.CharField(label='Full User Name',max_length=50)
    password1 = forms.CharField(label='password', widget=forms.PasswordInput)  
    password2 = forms.CharField(label='Confirm password', widget=forms.PasswordInput)  
    phone_number = forms.CharField(max_length=16, validators=[
    ...
    def save(self, commit=True):
        user = super(DeliverySignUpForm, self).save(commit=False)
        user.is_delivery = True
        if commit:
            user.save()
            delivery = Delivery(user=user,account_name = self.cleaned_data['account_name'])
            delivery.save()
        return user
...
class StoreEvaluationForm(forms.ModelForm):
    class Meta:
